...
class rom2dec
{
    ...
    public static function convert($string) {
        ...
        for ($i = 0; $i < strlen($string); ++$i) {
            ...
            if (!isset(self::$_FSM_table[$state][$symbol]) || self::$_FSM_table[$state][$symbol] === false) {
                throw new \InvalidArgumentException("Invalid symbol at offset $i");
            }
            $result += self::$_FSM_table[$state][$symbol][0];
            $state = self::$_FSM_table[$state][$symbol][1];
        }
